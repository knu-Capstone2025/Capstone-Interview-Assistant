...
namespace InterviewAssistant.Web.Tests.Services
{
    ...
    public class ChatServiceTests
    {
        ...
        public void Setup()
        {
            ...
            _chatService = new ChatService(_apiClient, _logger);
        }
        ...
        public async Task SendMessageAsync_WhenApiFails_ReturnsNull()
        {
            ...
            _apiClient.SendMessageAsync(Arg.Any<ChatRequest>()).Returns((ChatResponse)null);
        ...
        // 테스트 4: 채팅 초기화 메서드가 API 클라이언트의 초기화 메서드를 호출하는지 검증
        [Test]
        public async Task ResetChatAsync_CallsApiClientResetMethod()
        {
            // Arrange
            _apiClient.ResetChatAsync().Returns(Task.CompletedTask);
            
            // Act - ChatService의 채팅 초기화 메서드 호출
            await _chatService.ResetChatAsync();
            
            // Assert - API 클라이언트의 ResetChatAsync 메서드가 호출되었는지 확인
            await _apiClient.Received(1).ResetChatAsync();
        }
    }
}